...
class Dashboard extends BaseController
{
    public function index()
    {
        ...
        $data['multidata'] = $this->dashboard_model->get_cata_wais_transections();
    ...
    {
        $year  =  $this->request->getVar('yearly_investment');

        $monthlyInvestment = $this->dashboard_model->monthlyInvestment(@$year);
    ...
    public function bdtasktest()
    {
        ...
        if ($this->request->isAJAX())
        {
            ...
            $year  =  $this->request->getPost('yearly_investment');
            ...
            echo json_encode(array('investamount'=>$investamount,'months'=>$months, 'csrf_token' => $csrf_token, 'csrf_hash' => $csrf_hash));
        }
    }
    ...
    { 
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            $this->validation->setRule('firstname', display('first_name'),"max_length[100]|required|trim");
            $this->validation->setRule('lastname', display('last_name'),"max_length[100]|required|trim");
            $this->validation->setRule('email', display('email'),"max_length[100]|required|trim");
            $this->validation->setRule('about', display('about'),"max_length[100]|required|trim");
            $this->validation->setRule('image', 'Image', "ext_in[image,png,jpg,gif,ico]|is_image[image]");
            ...
            if(empty($img->getClientExtension())){
                ...
                $image = 'public/upload/settings/'.($image);
            }
            ...
            if($this->validation->withRequest($this->request)->run()){
                ...
                if (empty($userData['image'])) {
                    $this->session->setflashdata('exception', "Your have not selected any image!"); 
                }
