...
class SettingController extends BaseController
{
    ...
    public function index(){
        ...
        if($this->validation->withRequest($this->request)->run()){
            ...
            $logo_image_path = $this->imageupload->upload_image($this->request->getFile('logo'), 'public/upload/settings/', $this->request->getPost('old_logo'), 155, 50);
        ...
        if($this->validation->withRequest($this->request)->run()){
            ...
            $favicon_image_path = $this->imageupload->upload_image($this->request->getFile('favicon'), 'public/upload/settings/', $this->request->getPost('old_favicon'), 35, 35);
        ...
        if($this->validation->withRequest($this->request)->run()){
            ...
            $logo_web_image_path = $this->imageupload->upload_image($this->request->getFile('logo_web'), 'public/upload/settings/', $this->request->getPost('old_web_logo'), 155, 50);
    ...
    public function fees_setting_save()
    {
        ...
        $this->validation->setRule('fees', "Fees Percentage",'required|trim|max_length[20]');

        if ($this->validation->withRequest($this->request)->run()) {
      
            $check = $this->common_model->findById('dbt_fees', array('level' => $this->request->getPost('level')));


            if($check){

                $this->session->setFlashdata('exception', "The ".$this->request->getPost('level')." Level is already Exist.");
    ...
    public function transaction_setup(){
        ...
        $data['pager']     = $this->pager->makeLinks($page_number, 25, $total, 'front_full');
                ...
                if ($this->db->fieldExists("phrase", "language")) {
                    ...
                    foreach ($lang as $value) {
                        ...
                        if (!empty($value)) {
                            ...
                            $this->session->setFlashdata('exception', 'Please enter the phrase');
    ...
    private function create_and_write()
    {
        ...
        $result = $this->common_model->findAll('language', array(), 'id', 'asc', 2000, 0);
      
        $cache_file = './public/assets/js/language.json';
        $newarray = [];
    ...
    public function extrnal_api_list(){
        ...
        $data['pager'] = $this->pager->makeLinks($page_number, 20, $total, 'front_full');
