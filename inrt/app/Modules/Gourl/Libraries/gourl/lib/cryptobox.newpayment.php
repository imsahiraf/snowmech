...
function cryptobox_new_payment($paymentID = 0, $payment_details = array(), $box_status = "")
{
	...
	if (isset($payment_details['status']) && @$payment_details['status'] == 'payment_received' && $box_status == "cryptobox_newrecord") {
		...
		if (!$recordExists) {
			...
			run_sql("INSERT INTO `dbt_deposit` VALUES(NULL,'".$payment_details["user"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'bitcoin',".floatval($fees).",'".$paymentID."','".gmdate("Y-m-d H:i:s")."','".gmdate("Y-m-d H:i:s")."',NULL,".intval(1).",'".@$_SERVER['REMOTE_ADDR']."','".$payment_details["coinname"]."')");
