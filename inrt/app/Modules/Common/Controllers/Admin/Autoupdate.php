...
if(@fopen("https://update.bdtask.com", "r")) 
{
    ...
    //Get Update file
    define('MIN_VERSION', file_get_contents('https://update.bdtask.com/icowallet/autoupdate/update_min_version'));
    //Get Update file
    define('MAX_VERSION', file_get_contents('https://update.bdtask.com/icowallet/autoupdate/update_max_version'));

    //Get Update file
    define('UPDATE_URL','https://update.bdtask.com/icowallet/autoupdate');
...
class Autoupdate extends BaseController
{
    ...
    public function index(){ 
        ...
        if ($data['current_version'] >= $data['latest_version']) {
            $data['message_txt'] = '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Update not available';
        } else {

            $data['message_txt'] = '<i class="fa fa-exclamation-triangle" aria-hidden="true"></i>Version '.$data['latest_version'].' available';
        }  
    ...
    public function update()
    {
        if (!$this->session->get('isLogIn')&& !$this->session->get('isAdmin'))
           return redirect()->route('backend/autoupdate/autoupdate');
